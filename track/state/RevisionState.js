...
class RevisionState extends TrackState {
    ...
    submitReview(publication, score, message, reviewer) {
        ...
        if (!this.isValidateScore(score)) {
            throw new Error(`${reviewer.name} ${reviewer.lastName} no envio un score válido para la publicación "${publication.title}"`)
        }

        if (this.isNullOrEmpty(message)) {
            throw new Error(`${reviewer.name} ${reviewer.lastName} no envio un mensaje válido para la publicación "${publication.title}"`)
        }
    ...
    hasAlreadySendReview(publication, reviewer) {
        ...
        if (existingRev) {
            throw new Error(`El revisor ${reviewer.name} ${reviewer.lastName} ya envio una revisión a la publibicación "${publication.title}".`);
        }
    }
